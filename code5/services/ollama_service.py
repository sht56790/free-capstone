...
# Ollama 설정
OLLAMA_API_URL = "http://localhost:11434/api/chat"  # messages 형식 지원을 위해 /api/chat 사용
OLLAMA_API_URL_GENERATE = "http://localhost:11434/api/generate"  # prompt 형식용 (레거시)
...
def _post_ollama_generate(
    ...
    *,
    ...
    # messages 형식이면 /api/chat 사용, prompt 형식이면 /api/generate 사용
    if messages is not None:
        # messages 형식: /api/chat 엔드포인트 사용
        ...
        # prompt 형식: /api/generate 엔드포인트 사용 (레거시)
        payload: Dict[str, Any] = {
            "model": model,
            "prompt": prompt,
            "stream": False,
        ...
        if as_json:
            payload["format"] = "json"
        api_url = OLLAMA_API_URL_GENERATE
    ...
    # /api/generate 응답 형식: {"response": "..."}
    if messages is not None and "message" in result:
        # /api/chat 응답을 /api/generate 형식으로 변환
...
Example 7 - False positive prevention (CRITICAL):
Input: "다시 시도해주시거나"
Step 1: General instruction phrase "시도해주시거나" (try again)
Step 2: NO CONTACT/LOCATION - "시도해주세요" means "please try", NOT an address
Step 3: Common Korean verb phrase, not location data
Step 4: P0 - not sensitive (general instruction)
Step 5: IGNORE - DO NOT flag as ADDRESS
Output: []
⚠️ CRITICAL: "시도해주세요", "시도해주시거나", "주시거나" are VERB PHRASES, NOT addresses!

Example 8 - False positive prevention:
Input: "안내해주시거나"
Step 1: General instruction phrase
Step 2: NO CONTACT/LOCATION - common verb ending
Step 3: Not address data
Step 4: P0 - not sensitive
Step 5: IGNORE
Output: []
...
2. FALSE POSITIVE PREVENTION for ADDRESS:
   ✗ DO NOT flag Korean verb phrases as addresses:
     - "시도해주세요", "시도해주시거나" (please try) → IGNORE
     - "안내해주세요", "안내해주시거나" (please guide) → IGNORE
     - "주시거나", "주세요" (please give/do) → IGNORE
     - "확인해주세요" (please check) → IGNORE
   ✗ DO NOT flag single syllables "주" or "시" as addresses
   ✓ ONLY flag actual location addresses:
     - Must contain: 지역명(서울, 부산 등) + "시/도/군/구" + "로/길" + 번지
     - Or: 우편번호 패턴 (5자리-6자리 숫자)
   Examples:
   • "다시 시도해주시거나" → IGNORE (verb phrase, not address)
   • "서울시 강남구 테헤란로 123" → FLAG (real address)
   • "12345-67890" → FLAG (postal code)
