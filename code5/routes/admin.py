...
def get_trends():
    ...
    for log in logs:
        ...
        if log_date_str not in data_map:
            continue

        data_map[log_date_str]['total'] += 1

        # /dashboard-stats 와 동일한 기준: detections_in/out 에 값이 있는 로그는 모두 탐지로 간주
        has_detection = (
            isinstance(log.detections_in, list) and len(log.detections_in) > 0
        ) or (
            isinstance(log.detections_out, list) and len(log.detections_out) > 0
        )
        if has_detection:
...
# 🔄 Ollama 캐시 초기화 API
@admin_bp.post("/clear-ollama-cache")
def clear_ollama_cache():
    """Ollama 캐시를 초기화합니다."""
    try:
        from services.ollama_cache import clear_cache, get_cache_stats
...
def upload_documents():
    """PDF 파일을 업로드하고 벡터 DB를 즉시 재구축합니다."""
    import os
    from pathlib import Path
    ...
    # RAG 캐시 초기화 및 벡터 DB 재구축
    try:
        from services.rag_service import reset_vector_db
        reset_vector_db()
        
        # 기존 vector_db 폴더를 즉시 이름 변경 (파일 잠금 우회)
        vector_db_path = Path("vector_db")
        if vector_db_path.exists():
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            backup_path = Path(f"vector_db_old_{timestamp}")
            try:
                os.rename(vector_db_path, backup_path)
                print(f"[UPLOAD] 기존 vector_db 이름 변경: {backup_path}")
            except Exception as rename_err:
                print(f"[UPLOAD] vector_db 이름 변경 실패: {rename_err}")
        
        # 벡터 DB 재구축 (백그라운드에서 실행)
        script_path = Path("scripts/embed_documents.py")
        subprocess.Popen([sys.executable, str(script_path)])
        print(f"[UPLOAD] Vector DB 재구축 시작")
    except Exception as e:
        print(f"[UPLOAD] Vector DB 재구축 트리거 실패: {e}")
    
    return jsonify({
        "message": f"{len(uploaded_files)}개 파일이 업로드되었습니다.",
        "files": uploaded_files,
        "errors": errors if errors else None,
        "note": "벡터 DB가 초기화되었습니다. 잠시 후 재구축이 완료됩니다."
    })

@admin_bp.delete("/documents/<string:filename>")
def delete_document(filename):
    """특정 PDF 파일을 삭제하고 벡터 DB를 즉시 업데이트합니다."""
    import os
    from pathlib import Path
    from werkzeug.utils import secure_filename
    import subprocess
    ...
    try:
        # 1. PDF 파일 삭제
        filepath.unlink()
        print(f"[DELETE] PDF 파일 삭제됨: {safe_filename}")
        
        # 2. RAG 캐시 초기화
        from services.rag_service import reset_vector_db
        reset_vector_db()
        
        # 3. 기존 vector_db 폴더를 즉시 이름 변경 (파일 잠금 우회)
        vector_db_path = Path("vector_db")
        if vector_db_path.exists():
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            backup_path = Path(f"vector_db_old_{timestamp}")
            try:
                os.rename(vector_db_path, backup_path)
                print(f"[DELETE] 기존 vector_db 이름 변경: {backup_path}")
            except Exception as rename_err:
                print(f"[DELETE] vector_db 이름 변경 실패: {rename_err}")
        
        # 4. 벡터 DB 재구축 (백그라운드에서 실행)
        script_path = Path("scripts/embed_documents.py")
        subprocess.Popen([sys.executable, str(script_path)])
...
def rebuild_rag():
    ...
    try:
        ...
        from services.rag_service import reset_vector_db
        reset_vector_db()
        print("[REBUILD] RAG 캐시 초기화 완료")
        
        # 2. 기존 vector_db 폴더를 즉시 이름 변경 (파일 잠금 우회)
        vector_db_path = Path("vector_db")
        if vector_db_path.exists():
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            backup_path = Path(f"vector_db_old_{timestamp}")
            try:
                import os
                os.rename(vector_db_path, backup_path)
                print(f"[REBUILD] 기존 vector_db 이름 변경: {backup_path}")
            except Exception as rename_err:
                print(f"[REBUILD] vector_db 이름 변경 실패: {rename_err}")
        ...
        is_success = result.returncode == 0 or any(indicator in result.stdout for indicator in success_indicators)
        ...
        if result.stderr:
            ...
            for line in stderr_lines:
                if 'Error' in line or 'Exception' in line or 'Traceback' in line:
                    if 'DeprecationWarning' not in line and 'LangChainDeprecationWarning' not in line:
