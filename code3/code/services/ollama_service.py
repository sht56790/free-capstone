...
def _post_ollama_generate(model: str, messages: List[Dict[str, str]], *, as_json: bool, timeout: int = 30) -> Dict[str, Any]:
    """Ollama API에 POST 요청을 보냅니다 (messages 리스트 지원)."""
    payload = {
        "model": model,
        "messages": messages, # <-- prompt 대신 messages 리스트 사용
        "stream": False,
    }
    if as_json:
        payload["format"] = "json"
...
JSON Array:"""
    ...
    try:
        response_data = _post_ollama_generate(
            model or OLLAMA_MODEL_FOR_JUDGE,
            messages=messages_payload, # messages 인자로 전달
            as_json=True,
            timeout=30
        )
