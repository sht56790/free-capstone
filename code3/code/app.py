...
def create_app():
    ...
    with app.app_context():
        ...
        if api_key:
            system_instr_str =(
                    ...
                    "6. 면책 문구 포함: 모든 답변 끝에 '⚠️ 참고용, 실제 상품 가입·거래는 영업점 방문 또는 상담원 연결이 필요합니다.' 문구를 포함하세요.\n\n"
                    "7. 입력 요약 작성 규칙: 답변 섹션의 '입력 요약'에는 사용자 입력을 요약하되, 민감정보는 반드시 마스킹/일반화 처리하세요.\n"
                    "- 이름: '홍길동' → '홍OO' (성+OO)\n"
                    ...
                    "8. 고정 템플릿 사용(제목은 그대로 유지). 직원 관점의 구성:\n"
    ...
    def chat():
        ...
        try:
            # --- 1, 2, 3. FormData 읽기, 파일 처리, messages 파싱 ---
            ...
            orig = final_user_content if final_user_content else "" 
            ...
            try: 
                ...
                if judgements:
                     ...
                     for j in judgements:
                          ...
                          if "ACCOUNT" in pattern_labels and lbl == "PHONE":
                               account_pattern = re.compile(r"\d{3}[-\\s]?\d{3}[-\\s]?\d{6}|\d{4}[-\\s]?\d{4}[-\\s]?\d{4,6}|\d{10,14}")
                ...
                # 5. RAG 검색 (필터링된 안전한 질문으로 Context 검색)
            ...
            if not is_blocked:
                ...
                if model_id.startswith("ollama:"):
                     # ▼▼▼ Ollama용 시스템 프롬프트를 여기서 정의 ▼▼▼
                     final_system_instruction = (
                         "당신은 '금융회사 직원 보조용' 상담 에이전트입니다. "
                         "모든 답변은 다음 [참고 자료]에 근거해야 합니다.\n\n"
                         "참고 자료에 내용이 없으면 '내부 자료에서 관련 정보를 찾을 수 없습니다'라고 답변하세요.\n"
                         "금융 상담 템플릿(## 답변, ## 근거 출처 등)을 반드시 준수하세요.\n\n"
                         "--- [검색된 참고 자료] ---\n"
                         f"{context}\n"
                         "--------------------------\n"
                         # (필요시 Gemini 프롬프트의 금융 지침을 여기에 추가)
                     )
                     llm_resp = call_ollama_generate(model_id, sanitized_messages, system_instruction=final_system_instruction)
                
                elif model_id == "demo-local":
                     llm_resp = call_gemini_generate(model_id, sanitized_messages, app.GMODEL) 
                
                else: # Gemini 또는 기타 모델
                     # ▼▼▼ Gemini용 시스템 프롬프트를 여기서 정의 ▼▼▼
                     if not (hasattr(app, 'GMODEL') and app.GMODEL):
                          return jsonify({"error": "Gemini 모델이 설정되지 않았습니다.", "detail": "GOOGLE_API_KEY 확인 필요"}), 503
                     
                     final_system_instruction = ORIGINAL_GEMINI_SYSTEM_INSTRUCTION + "\n\n[검색된 내부 참고 자료]:\n" + context
                     
                     gmodel_with_rag = genai.GenerativeModel(
                          model="gemini-2.5-pro", # 또는 app.GMODEL.model
                          system_instruction=final_system_instruction
                     )
                     llm_resp = call_gemini_generate(model_id, sanitized_messages, gmodel_with_rag)
                
                # ... (이하 후처리 및 로그 준비 로직은 기존과 동일) ...
        ...
        # ... (이하 except 및 finally, 응답 반환 로직은 기존과 동일) ...
        except json.JSONDecodeError:
             traceback.print_exc()
             return jsonify({"error": "잘못된 messages 형식"}), 400
        except Exception as e: 
             print(f"An error occurred in /chat processing: {e}")
             traceback.print_exc()
             log_data = { 
             ...
             try:
                  log_data.update({ "user": session.get('user_id'), "user_prompt": orig })
        ...
        if is_blocked:
             block_reason_kr = {
                  "계좌번호": "계좌번호", "고객번호": "고객번호", "전화번호": "전화번호", "주소": "주소",
                  "ACCOUNT": "계좌번호", "CUSTOMER_ID": "고객번호", "PHONE": "전화번호", "ADDRESS": "주소",
             ...
             _label_name = {
                  "ACCOUNT": "계좌번호", "CUSTOMER_ID": "고객번호", "PHONE": "전화번호", "NAME": "고객명", "ADDRESS": "주소", "DOB": "생년월일",
                  "계좌번호": "계좌번호", "고객번호": "고객번호", "전화번호": "전화번호", "고객명": "고객명", "주소": "주소", "생년월일": "생년월일",
             }
             detected_names = []
             ...
             disclaimer = "⚠️ 참고용, 실제 상품 가입·거래는 영업점 방문 또는 상담원 연결이 필요합니다."
             if disclaimer not in sanitized_out:
...
def call_gemini_generate(model_id: str, messages: List[Dict[str, str]], gmodel) -> str:
    if not (model_id and model_id != "demo-local" and not model_id.lower().startswith("gpt-")):
        ...
        summary = (last.get("content") or "")[:120]
        return (
            "## 답변\n"
            f"- 입력 요약: {summary}\n"
...
def call_ollama_generate(model_id: str, messages: List[Dict[str, str]], system_instruction: str) -> str:
    """Ollama 텍스트 생성 (RAG Context를 인자로 받음)."""
    
    # 1. 모델 이름 추출
    model = model_id.split(":", 1)[1] if ":" in model_id else model_id
    
    # 2. Ollama API 호출을 위한 메시지 목록 구성
    ollama_messages = []
    
    # a. 시스템 지침 (RAG Context 포함)을 메시지 목록 맨 처음에 추가
    ollama_messages.append({"role": "system", "content": system_instruction})

    # b. 대화 히스토리 및 현재 질문 추가
    for m in messages:
    ...
    # 3. Ollama API 호출 (services._post_ollama_generate 사용)
    try:
...
def _dedupe_masked_vs_raw_lines(text: str) -> str:
    ...
    account_rx = re.compile(r"\d{3}[-\s]?\d{3}[-\s]?\d{6}|\d{4}[-\s]?\d{4}[-\s]?\d{4,6}|(?<!0)\d{10,14}")
    ...
    for i, line in enumerate(lines):
        ...
        if line_words and summary_words:
            ...
            if overlap_ratio > 0.7 and any(rx.search(line) for rx in [
                re.compile(r"\d{3}[-\s]?\d{3}[-\s]?\d{6}|\d{4}[-\s]?\d{4}[-\s]?\d{4,6}|(?<!0)\d{10,14}"),
...
def create_masked_summary(text: str) -> str:
    ...
    account_with_dash = re.compile(r"\d{3}[-\\s]?\d{3}[-\\s]?\d{6}|\d{4}[-\\s]?\d{4}[-\\s]?\d{4,6}")
