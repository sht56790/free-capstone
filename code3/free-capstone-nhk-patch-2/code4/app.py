...
def call_ollama_generate(model_id: str, messages: List[Dict[str, str]]) -> str:
    ...
    # 전체 프롬프트 구성: 히스토리 + 현재 메시지 + 지시사항
    full_prompt = context + f"사용자: {last_user}\n\n어시스턴트: (위 지침의 고정 템플릿을 반드시 사용하세요)"
    
    try:
        data = _post_ollama_generate(model=model, prompt=full_prompt, as_json=False, timeout=30)
