...
# from services.search_service import search_and_format  # Google Search 기능 제거
...
def create_app():
    ...
    with app.app_context():
        ...
        if api_key:
            ...
            system_instr_str = (
                    "당신은 '금융회사 직원 전용 업무 보조 AI'입니다. 답변 대상은 '직원'이며, 간결하고 명확한 정보 제공이 목표입니다.\n\n"
                    ...
                    "📋 **답변 형식 (제공된 정보의 성격에 따라 선택)**\n\n"
                    
                    "1️⃣ **실시간 정보가 제공된 경우** (주가 데이터, 날씨 정보, 환율 등)\n"
                    "   → 간단하고 직접적으로 답변하세요.\n"
                    "   예: '현재 코스피 지수는 2,500포인트로 전일 대비 1.2% 상승했습니다.'\n\n"
                    
                    "2️⃣ **금융 상담 정보가 제공된 경우** (대출, 예금, 카드, 업무 절차 등)\n"
                    "   → 아래 템플릿을 사용하세요:\n"
                    "   ## 입력 요약\n"
                    "   [사용자 질문을 1-2문장으로 요약]\n\n"
                    "   ## 답변\n"
                    "   [핵심 내용을 명확하게 설명. 필요시 단계별로 구분]\n\n"
                    "   ## 참고 정보\n"
                    "   - [추가 정보나 주의사항]\n"
                    "   - [관련 문서 출처가 있다면 명시]\n\n"
                    
                    "3️⃣ **일반 지식 질문** (정보 제공 없음)\n"
                    "   → 간단하고 친절하게 답변하세요. 템플릿 불필요.\n"
                    "   예: 'Python의 print 함수는 콘솔에 텍스트를 출력하는 함수입니다. (일반 지식 기반)'\n\n"
            ...
            app.GMODEL = genai.GenerativeModel(
                "models/gemini-2.5-flash",  # 최신 Gemini 2.5 Flash
    ...
    def chat():
        print("=" * 80)
        print(f"[REQUEST] /chat 엔드포인트 호출됨")
        print("=" * 80)
        
        if 'user_id' not in session:
            print(f"[REQUEST] 로그인 필요 - 세션에 user_id 없음")
            return jsonify({"error": "로그인이 필요합니다.", "detail": "세션 만료"}), 401
        
        print(f"[REQUEST] 사용자 ID: {session.get('user_id')}")
        model_id = request.form.get("model", "gemini-1.5-flash")
        print(f"[REQUEST] 선택된 모델: {model_id}")
        ...
        block_reason = ""
        context = ""
        has_relevant_docs = False  # RAG 관련 정보 유무 플래그
        source_docs = []  # 참고 문서 정보 초기화
        ...
        try:
            ...
            print(f"[REQUEST] 메시지 개수: {len(messages)}")
            
            final_user_content = user_prompt_text
            print(f"[REQUEST] 사용자 입력: {user_prompt_text[:100]}...")
            
            if file_content:
                final_user_content = f"{user_prompt_text}\n\n--- 첨부 파일 내용 ---\n{file_content}".strip()
                print(f"[REQUEST] 파일 첨부됨 - 최종 입력 길이: {len(final_user_content)}")
            
            print(f"[REQUEST] 최종 사용자 내용 길이: {len(final_user_content)} 문자")
            ...
            try:  # <-- ValueError 발생 가능 구간 시작
                ...
                staff_mode = session.get('role') in ['admin', 'user']  # 로그인한 사용자는 직원 모드
                if staff_mode:
                    for j in judgements:
                ...
                rag_context, similarity_score, source_docs = retrieve_context(orig)
                
                # 관련 정보 유무 판단: 유사도 점수 기반 판단
                # 유사도가 0.5 이상이면 관련 정보 있음, 그 이하면 관련 정보 없음으로 간주
                SIMILARITY_THRESHOLD = 0.5
                has_relevant_docs = (
                    rag_context 
                    and not rag_context.startswith("참고:") 
                    and len(rag_context.strip()) > 50
                    and "--- 참고자료" in rag_context
                    and similarity_score >= SIMILARITY_THRESHOLD
                )
                
                print(f"[RAG] 검색된 문서 유사도 점수: {similarity_score:.3f} (임계값: {SIMILARITY_THRESHOLD})")
                
                # 유사도가 임계값 이하면 source_docs를 비움 (클라이언트로 전송하지 않음)
                if similarity_score < SIMILARITY_THRESHOLD:
                    print(f"[RAG] 유사도가 임계값 이하 - 참고 문서 정보를 클라이언트로 전송하지 않음")
                    source_docs = []
                
                if has_relevant_docs:
                    # 관련 정보가 있을 때: 내부 문서만 사용, 외부 검색 제외
                    print(f"[RAG] 내부 문서 기반 답변 모드 (관련 문서 발견)")
                    context = rag_context
                else:
                    # 관련 정보가 없을 때: 모델에 따라 외부 정보 수집
                    print(f"[RAG] 일반 모드 (내부 문서 없음)")
                    context = rag_context if rag_context else ""
                    
                    # 외부 정보 수집 전략
                    # 1순위: 실시간 API (날씨, 주가 등)
                    # 2순위: 웹 스크래핑 (Ollama만)
                    # LLM 자체 지식 활용 (Gemini는 강력한 일반 지식 보유)
                    
                    print(f"[검색 전략] 외부 정보 수집 시작")
                    
                    # 1순위: 실시간 API (모든 모델 공통)
                    realtime_info = detect_and_fetch_realtime_info(orig)
                    if realtime_info:
                        print(f"[REALTIME API] 실시간 정보 추가")
                        context = f"{realtime_info}\n\n{context}" if context else realtime_info
                    
                    # 2순위: 웹 스크래핑 (Ollama만, 지식이 제한적이므로)
                    if model_id.startswith("ollama:") and not context:
                        print(f"[검색 전략] Ollama 모델 - 웹 스크래핑 시도")
                        scraped_info = smart_scrape(orig)
                        if scraped_info:
                            print(f"[WEB SCRAPE] 스크래핑 정보 추가")
                            context = scraped_info
                        else:
                            print(f"[INFO] 스크래핑 정보 없음 - Ollama 자체 지식 활용")
                    elif not model_id.startswith("ollama:"):
                        print(f"[INFO] Gemini/외부 모델 - 자체 지식 활용")
            ...
            if not is_blocked:
                ...
                # 보안: 관련 정보가 있을 때는 자동으로 로컬 모델(Ollama) 사용
                # 사용자가 외부 LLM을 선택했어도 내부 문서 보호를 위해 로컬 모델로 자동 전환
                if has_relevant_docs and not model_id.startswith("ollama:"):
                    # 내부 문서가 있는데 외부 LLM을 선택한 경우 로컬 모델로 자동 전환
                    default_local_model = "ollama:qwen3:8b"  # 기본 로컬 모델 (프론트엔드에서 사용 가능한 모델)
                    print(f"[보안] 내부 문서 발견 - 외부 LLM({model_id}) 대신 로컬 모델({default_local_model})로 자동 전환")
                    model_id = default_local_model
                
                # 모델 라우팅: ollama:* 은 Ollama로, 그 외는 기존 로직
                print(f"[DEBUG] 모델 라우팅 시작 - model_id: {model_id}, has_relevant_docs: {has_relevant_docs}")
                if model_id.startswith("ollama:"):
                    model_used = model_id  # Ollama 모델 사용
                    
                    if has_relevant_docs:
                        # 관련 정보가 있을 때: 내부 문서만 참조하도록 강제 (로컬 모델만 사용)
                        final_system_instruction = (
                            "당신은 금융회사 직원 보조용 AI 어시스턴트입니다.\n\n"
                            "**중요: 반드시 아래 [내부 문서]에 있는 정보만 사용하여 답변하세요.**\n"
                            "- 문서에 있는 구체적인 수치, 조건, 상품명을 그대로 사용하세요.\n"
                            "- 문서에 없는 내용은 언급하지 마세요.\n"
                            "- 답변의 근거가 되는 문서 출처를 반드시 명시하세요.\n\n"
                            "답변 형식:\n"
                            "## 입력 요약\n"
                            "[사용자 질문을 1-2문장으로 요약]\n\n"
                            "## 답변\n"
                            "[내부 문서의 구체적인 정보를 바탕으로 명확하게 설명. 금리, 조건, 상품명 등 문서에 있는 구체적인 내용 포함]\n\n"
                            "## 참고 정보\n"
                            "- 출처: [참고한 문서명 명시]\n"
                            "- [문서의 추가 정보나 주의사항]\n\n"
                            "========================================\n"
                        ...
                        # 관련 정보가 없을 때: 검색 결과/실시간 정보/일반 지식 활용
                        # LLM이 context 내용을 보고 스스로 적절한 형식 선택
                        final_system_instruction = (
                            "당신은 '금융회사 직원 보조용' AI 어시스턴트입니다.\n\n"
                            "아래 제공된 정보를 바탕으로 답변하세요. 정보가 없으면 일반 지식으로 답변하세요.\n\n"
                            "**답변 형식 선택 기준:**\n"
                            "1. 실시간 정보 (주가, 날씨, 환율 등): 간단하고 직접적으로 답변\n"
                            "   예: '현재 코스피 지수는 2,500포인트로 전일 대비 1.2% 상승했습니다.'\n\n"
                            "2. 금융 상담 질문 (대출, 예금, 카드 등): 아래 템플릿 사용\n"
                            "   ## 입력 요약\n"
                            "   [사용자 질문을 1-2문장으로 요약]\n\n"
                            "   ## 답변\n"
                            "   [핵심 내용을 명확하게 설명]\n\n"
                            ...
                            "   - [추가 정보나 주의사항]\n"
                            "   - [관련 문서 출처가 있다면 명시]\n\n"
                            "3. 일반 질문: 간결하고 친절하게 자유 형식으로 답변\n\n"
                            "--- [참고 정보] ---\n"
                            f"{context}\n"
                            "--------------------------\n"
                        )
                    
                    print(f"[Ollama] 모델: {model_id}, 관련 문서: {'있음' if has_relevant_docs else '없음'}")
                    print(f"[Ollama] System instruction 길이: {len(final_system_instruction)} 문자")
                    print(f"[Ollama] Context 길이: {len(context)} 문자")
                    llm_resp = call_ollama_generate(
                        model_id,
                        sanitized_messages,
                        system_instruction=final_system_instruction
                    )
                    print(f"[Ollama] 응답 길이: {len(llm_resp)} 문자")
                    if not llm_resp or llm_resp.strip() == "(empty response)":
                        print(f"[Ollama] ⚠️ 빈 응답 감지! 응답 내용: {repr(llm_resp)}")
                else:
                    print(f"[DEBUG] Gemini 라우팅 - model_id: {model_id}")
                    if model_id == "demo-local":
                        print(f"[DEBUG] demo-local 모드 실행")
                        model_used = "demo-local (fallback)"
                        llm_resp = call_gemini_generate(model_id, sanitized_messages, app.GMODEL, context=context)
                    else:
                        print(f"[DEBUG] Gemini API 호출 준비 - ORIGINAL_GEMINI_SYSTEM_INSTRUCTION 존재: {bool(ORIGINAL_GEMINI_SYSTEM_INSTRUCTION)}")
                        if not ORIGINAL_GEMINI_SYSTEM_INSTRUCTION:
                            return jsonify({"error": "Gemini 모델 지침이 설정되지 않았습니다.", "detail": "서버 설정 오류"}), 503
                        
                        # 지원되는 Gemini 텍스트 모델 목록 (프론트엔드 값)
                        supported_gemini_models = {
                            "gemini-1.5-flash": "models/gemini-2.5-flash",  # 1.5는 없으므로 2.5로 매핑
                            "gemini-2.0-flash": "models/gemini-2.0-flash",
                            "gemini-2.0-flash-lite": "models/gemini-2.0-flash-lite",
                        }
                        
                        # 프론트엔드 모델 ID를 API 모델 이름으로 변환
                        if model_id in supported_gemini_models:
                            gemini_model_name = supported_gemini_models[model_id]
                        else:
                            gemini_model_name = "models/gemini-2.5-flash"  # 기본값 (최신)
                        
                        model_used = gemini_model_name
                        # 디버깅: 컨텍스트 내용 출력
                        print(f"[CONTEXT] Gemini에게 전달되는 컨텍스트 (처음 500자):\n{context[:500] if context else '(없음)'}")
                        
                        # 관련 정보가 있을 때는 이미 위에서 차단되었으므로 여기서는 관련 정보가 없는 경우만 처리
                        # 일반 지식 및 검색 결과 허용
                        final_system_instruction = (
                            ORIGINAL_GEMINI_SYSTEM_INSTRUCTION
                            + "\n\n[검색된 내부 참고 자료]:\n"
                        ...
                        print(f"[DEBUG] 생성된 모델명: {gemini_model_name}")
                        print(f"[DEBUG] gmodel_with_rag.model_name: {gmodel_with_rag.model_name if hasattr(gmodel_with_rag, 'model_name') else 'N/A'}")
        ...
        if is_blocked:
            ...
            return jsonify({
                ...
                "source_docs": source_docs  # 참고 문서 정보 추가
            })
    # ==================================================================
...
def call_gemini_generate(
    ...
    *,
    ...
    if not (model_id and model_id != "demo-local" and not model_id.lower().startswith("gpt-")):
        return "안내는 영업점 방문 또는 상담원 연결로 진행됩니다."
    ...
    print(f"[Gemini API] 사용자 메시지: {last_user[:100]}...")
    ...
    try:
        ...
        if result_text:
            print(f"[Gemini API] 응답 미리보기: {result_text[:200]}...")
        ...
        # 안전성 필터 차단
        if hasattr(api_err, 'candidates') or 'blocked' in error_msg.lower() or 'SAFETY' in error_msg:
            print(f"[Gemini API] ⚠️ 안전성 필터에 의해 차단됨")
            return "해당 질문에 대해 답변을 생성할 수 없습니다. 다른 방식으로 질문해주세요."
        
        # API 키 오류
        if "403" in error_msg or "PermissionDenied" in error_msg or "leaked" in error_msg.lower():
            print(f"[Gemini API] ⚠️ API 키 문제")
            return "현재 시스템 점검 중입니다. 잠시 후 다시 시도해주세요."
        
        # 기타 오류
        return f"오류가 발생했습니다: {error_msg[:100]}"
...
def format_counselor_response(text: str, original_input: str = "", ner_results: Optional[List[Dict[str, Any]]] = None) -> str:
    """응답을 간결한 형식으로 정규화하고 입력 요약 부분을 마스킹합니다.
    ...
    if original_input:
        ...
        if "입력 요약" in t:
            ...
            # LLM이 입력 요약을 생성하지 않은 경우: 상단에 "## 입력 요약" 섹션 추가
            if "## 입력 요약" not in t:
                t = "## 입력 요약\n" + masked_summary + "\n\n" + t
