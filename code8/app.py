    ...
    with app.app_context():
        ...
        if api_key:
            ...
            app.GMODEL = genai.GenerativeModel(
                "models/gemini-1.5-flash",  # v1beta API 버전에 맞는 형식
    ...
    def chat():
        ...
        try:
            ...
            try:  # <-- ValueError 발생 가능 구간 시작
                ...
                # 유사도가 0.7 이상이면 관련 정보 있음, 그 이하면 관련 정보 없음으로 간주
                ...
                if model_id.startswith("ollama:"):
                    ...
                    if has_relevant_docs:
                        ...
                        final_system_instruction = (
                            ...
                            "아래 [내부 문서]를 참고하여 사용자의 질문에 답변하세요.\n\n"
                            "답변 형식:\n"
                            "## 답변\n"
                            "- 입력 요약: [사용자 질문 요약]\n"
                            "- 고객 응대 멘트: [내부 문서를 바탕으로 한 구체적인 답변 2-3문장]\n"
                            "- 참고 정보: [내부 문서에서 확인한 추가 정보]\n"
                        ...
                        final_system_instruction = (
                            ...
                            "**답변 형식 선택 기준:**\n"
                            "1. 실시간 정보 (주가, 날씨, 환율 등): 간단하고 직접적으로 1-2문장 답변\n"
                            "   예: '현재 코스피 지수는 2,500포인트로 전일 대비 1.2% 상승했습니다.'\n\n"
                    ...
                    if model_id == "demo-local":
                        ...
                        supported_gemini_models = {
                            ...
                            "gemini-2.0-flash": "models/gemini-2.0-flash-exp",  # 2.0은 exp 버전 사용
                        ...
                        if model_id in supported_gemini_models:
                            ...
                            gemini_model_name = "models/gemini-1.5-flash"  # 기본값 (안정적인 무료 티어)
...
def call_gemini_generate(
    ...
    *,
    ...
    try:
        ...
        # 안전성 필터 차단 확인
        if hasattr(api_err, 'candidates') or 'blocked' in error_msg.lower():
            print(f"[Gemini API] ⚠️ 안전성 필터에 의해 차단되었을 가능성")
        
        # API 키 관련 오류인지 확인
        if "403" in error_msg or "PermissionDenied" in error_msg or "leaked" in error_msg.lower():
            print(f"[Gemini API] ⚠️ API 키 문제 감지")
    ...
    if (not normalized) or normalized.lower() in {"(empty response)", "empty response"}:
        ...
        return (
            "## 답변\n"
            f"- 입력 요약: {fallback_summary}\n"
            "- 고객 응대 멘트: 내부 매뉴얼을 근거로 해당 절차와 수수료 범위를 안내해주세요.\n"
            "- 참고 정보: 상세한 내용은 내부 매뉴얼 또는 상담원을 통해 확인해주세요.\n"
            "- 내부 체크리스트: 필요한 서류 확인 → 상담 이력 기록 → 면책 문구 포함 안내\n\n"
            "## 근거 출처\n"
            f"- 참고 자료{context_hint}\n\n"
            "## 다음 단계\n"
            "- 상세 내용은 담당 부서에 문의하거나 내부 매뉴얼을 확인하세요."
        )
    return result_text
